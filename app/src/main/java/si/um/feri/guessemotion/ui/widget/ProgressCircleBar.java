package si.um.feri.guessemotion.ui.widget;
...
public class ProgressCircleBar extends ProgressBar {

    private static final String TAG = "ProgressCircleBar";
    ...
    public ProgressCircleBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public ProgressCircleBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void init(AttributeSet attrs, int style){
        ...
        TypedArray a = getContext().obtainStyledAttributes(attrs,
                R.styleable.ProgressCircleBar, style, 0);

        String color;
        Resources res = getResources();

        this.mHasShadow = a.getBoolean(R.styleable.ProgressCircleBar_cpb_hasShadow, true);

        color = a.getString(R.styleable.ProgressCircleBar_cpb_progressColor);
        if(color==null)
            mProgressColorPaint.setColor(res.getColor(R.color.progress_circle_default_progress));
        else
            mProgressColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.ProgressCircleBar_cpb_backgroundColor);
        if(color==null)
            mBackgroundColorPaint.setColor(res.getColor(R.color.progress_circle_default_background));
        else
            mBackgroundColorPaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.ProgressCircleBar_cpb_titleColor);
        if(color==null)
            mTitlePaint.setColor(res.getColor(R.color.progress_circle_default_title));
        else
            mTitlePaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.ProgressCircleBar_cpb_subtitleColor);
        if(color==null)
            mSubtitlePaint.setColor(res.getColor(R.color.progress_circle_default_subtitle));
        else
            mSubtitlePaint.setColor(Color.parseColor(color));


        String t = a.getString(R.styleable.ProgressCircleBar_cpb_title);
        if(t!=null)
            mTitle = t;

        t = a.getString(R.styleable.ProgressCircleBar_cpb_subtitle);
        if(t!=null)
            mSubTitle = t;

        mStrokeWidth = a.getInt(R.styleable.ProgressCircleBar_cpb_strokeWidth, STROKE_WIDTH);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if(!TextUtils.isEmpty(mTitle)){
            mTitlePaint.setTextSize(AndroidUtils.convertSpToPixels(35, getContext()));
            ...
            mSubtitlePaint.setTextSize(AndroidUtils.convertSpToPixels(15, getContext()));
            yPos += titleHeight;
    ...
    public void animateProgressTo(final int start, final int end, final ProgressAnimationListener listener){
        ...
        progressBarAnimator.setDuration(1000 * start);
        ...
        progressBarAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(final Animator animation) {
                ProgressCircleBar.this.setProgress(end);
                if(listener!=null)
        ...
        progressBarAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(final ValueAnimator animation) {
                ...
                if( progress != ProgressCircleBar.this.getProgress() ){
                    Log.d(TAG, progress + "");
                    ProgressCircleBar.this.setProgress(progress);
                    ProgressCircleBar.this.setTitle(String.valueOf(progress));
                    if(listener!=null)
    ...
    public void startAnimation() {
        progressBarAnimator.start();
        progressBarAnimator.setCurrentPlayTime(mCurrentPlayTime);
    }
    ...
    public void stopAnimation() {
        mCurrentPlayTime = progressBarAnimator.getCurrentPlayTime();
    ...
    public synchronized void setProgresBarColor(int color) {
